...
public class ContentProviderPlugin extends CordovaPlugin {
	...
	private void insertQuery(JSONObject queryArgs, CallbackContext callback) {
		...
		try {
			if (!queryArgs.isNull("userName")) {
				contentVal.put("USER_NAME",queryArgs.getString("userName"));
			}
		...
		try {
			if (!queryArgs.isNull("firstName")) {
				contentVal.put("FIRST_NAME",queryArgs.getString("firstName"));
			}
		} catch (JSONException e1) {
			contentVal.put("FIRST_NAME","");
		}

		try {
			if (!queryArgs.isNull("lastName")) {
				contentVal.put("LAST_NAME",queryArgs.getString("lastName"));
			}
		} catch (JSONException e1) {
			contentVal.put("LAST_NAME","");
		}

		try {
			if (!queryArgs.isNull("email")) {
				contentVal.put("EMAIL",queryArgs.getString("email"));
			}
		} catch (JSONException e1) {
			callback.error(WRONG_PARAMS);
			return;
		...
		try {
			if (!queryArgs.isNull("pass")) {
				contentVal.put("PASS",queryArgs.getString("pass"));
			}
		} catch (JSONException e1) {
			callback.error(WRONG_PARAMS);
			return;
		...
		//check content provider exists
		if(cursor == null) {
			callback.error(UNKNOWN_ERROR);
		} else {
			// run insert query
			response = cordova.getActivity().getContentResolver().insert(contentUri,contentVal);

			if(response == null) {
				callback.error(UNKNOWN_ERROR);
			} else {
				JSONObject responseCallBack = new JSONObject();
				try {
					try {
						responseCallBack.put("Status", "SUCCESS");
						responseCallBack.put("Message", "Data inserted successfully");
					} catch (JSONException e) {
				...
				callback.success(responseCallBack);
			}
		}
	}
